...
const mockContext: any = {
  ...liveContext,
  empathic: async ([name]: string[]) => name,
  fs: {
...
describe.skip('these tests need to be updated', () => {
  ...
  describe('Check Vitest workspace files', () => {
    it('should disallow JSON workspace file', async () => {
      const result = await vitestConfigFiles.condition(
        { ...mockContext, empathic: coerce('workspace', 'vitest.workspace.json') },
        state
      );
      expect(result).toEqual({
        type: 'incompatible',
    ...
    it('should disallow invalid workspace file', async () => {
      const result = await vitestConfigFiles.condition(
        { ...mockContext, empathic: coerce('workspace', 'invalidWorkspace.ts') },
        state
      );
      expect(result).toEqual({
        type: 'incompatible',
    ...
    it('should allow defineWorkspace syntax', async () => {
      const result = await vitestConfigFiles.condition(
        { ...mockContext, empathic: coerce('workspace', 'defineWorkspace.ts') },
        state
      );
      expect(result).toEqual({
        type: 'compatible',
    ...
    it('should disallow invalid defineWorkspace syntax', async () => {
      const result = await vitestConfigFiles.condition(
        { ...mockContext, empathic: coerce('workspace', 'defineWorkspace-invalid.ts') },
        state
      );
      expect(result).toEqual({
        type: 'incompatible',
  ...
  describe('Check Vitest config files', () => {
    it('should disallow CommonJS config file', async () => {
      const result = await vitestConfigFiles.condition(
        { ...mockContext, empathic: coerce('config', 'vitest.config.cjs') },
        state
      );
      expect(result).toEqual({
        type: 'incompatible',
    ...
    it('should disallow invalid config file', async () => {
      const result = await vitestConfigFiles.condition(
        { ...mockContext, empathic: coerce('config', 'invalidConfig.ts') },
        state
      );
      expect(result).toEqual({
        type: 'incompatible',
    ...
    it('should allow existing test config option', async () => {
      const result = await vitestConfigFiles.condition(
        { ...mockContext, empathic: coerce('config', 'testConfig.ts') },
        state
      );
      expect(result).toEqual({
        type: 'compatible',
    ...
    it('should disallow invalid test config option', async () => {
      const result = await vitestConfigFiles.condition(
        { ...mockContext, empathic: coerce('config', 'testConfig-invalid.ts') },
        state
      );
      expect(result).toEqual({
        type: 'incompatible',
    ...
    it('should allow existing test.workspace config option', async () => {
      const result = await vitestConfigFiles.condition(
        { ...mockContext, empathic: coerce('config', 'workspaceConfig.ts') },
        state
      );
      expect(result).toEqual({
        type: 'compatible',
    ...
    it('should disallow invalid test.workspace config option', async () => {
      const result = await vitestConfigFiles.condition(
        { ...mockContext, empathic: coerce('config', 'workspaceConfig-invalid.ts') },
        state
      );
      expect(result).toEqual({
        type: 'incompatible',
